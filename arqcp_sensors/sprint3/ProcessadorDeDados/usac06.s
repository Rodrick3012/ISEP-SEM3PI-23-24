...
equal:
    ...
    movb (%rdi, %r9, 1), %al   # Lê o caractere da string em %rdi
...
# Se chegou ao final da string ou encontrou um '#' (nao houve resultado final, retorna 0)
exit:
    movl $1431655765, (%rdx)   # Define um valor de sentinela no endereço apontado por %rdx
    movl $0, %eax	#retorna 0 quando nao foi encontrado token
    ...
    pop %r9                    # Restaura o índice da string em %rdi
    movl $1, %eax		#retorna 1 quando o token é encontrado
    ret                         # Retorna da função
